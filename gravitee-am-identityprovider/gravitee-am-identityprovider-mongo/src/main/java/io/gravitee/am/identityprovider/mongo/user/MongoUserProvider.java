...
public class MongoUserProvider implements UserProvider, InitializingBean {
   ...
   private MongoCollection<Document> usersCollection;
    ...
    public Single<User> create(User user) {
        ...
        // set technical id
        document.put(FIELD_ID, UUID.toString(UUID.random()));
        // set username
        document.put(FIELD_USERNAME, user.getUsername());
        // set password
    ...
    public Single<User> update(User user) {
        ...
        // set date fields
        document.put(FIELD_CREATED_AT, user.getCreatedAt());
        document.put(FIELD_UPDATED_AT, new Date());
        return Single.fromPublisher(usersCollection.replaceOne(eq(FIELD_ID, user.getId()), document)).flatMap(updateResult -> findById(user.getId()).toSingle());
    }

    @Override
    public Completable delete(String userId) {
        return Completable.fromPublisher(usersCollection.deleteOne(eq(FIELD_ID, userId)));
    }
    ...
    public void afterPropertiesSet() {
        ...
        Observable.fromPublisher(usersCollection.createIndex(new Document(FIELD_USERNAME, 1))).subscribe();
    }
